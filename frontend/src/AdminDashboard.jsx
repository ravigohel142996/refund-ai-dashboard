...
function RequestCard({ data, index }) {
  ...
  return (
    <div
      className="card-subtle rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-md hover:border-slate-200 group"
    ...
    >
      ...
      {/* Header */}
      <div
        className="px-5 py-4 flex items-center justify-between cursor-pointer select-none bg-white hover:bg-slate-50/60 transition-colors"
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-mono text-xs text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-md border border-indigo-100">
                {data.order_id}
              </span>
            </div>
            ...
            <div className="space-y-2">
              {data.agent_reasoning_logs?.map((log, i) => (
                <div key={i} className="flex gap-3 relative">
                  ...
                  {/* Content */}
                  <div className="flex-1 bg-white rounded-xl p-3 border border-slate-200 shadow-sm min-w-0">
      ...
      ws.onmessage = (event) => {
        try {
          ...
          setRequests((prev) => {
            ...
            const exists = prev.some(r => r.customer_id === data.customer_id && r.order_id === data.order_id && r.decision === data.decision);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/20 text-slate-800 font-sans relative overflow-x-hidden">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="float-orb absolute -top-32 left-[10%] w-[500px] h-[500px] bg-indigo-100/40 rounded-full blur-[120px]" />
        <div className="float-orb-delay absolute top-1/3 right-[5%] w-[400px] h-[400px] bg-violet-100/30 rounded-full blur-[120px]" />
      </div>

      {/* ── Navbar ── */}
      <nav className="sticky top-0 z-50">
        <div className="nav-surface px-6 py-3.5 flex items-center justify-between max-w-7xl mx-auto">
          {/* Brand */}
          <div className="flex items-center gap-3">
